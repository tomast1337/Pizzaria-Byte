...
>('gerirUsers/fetchUserPedidos', async (email, { rejectWithValue }: any) => {
    try {
        const token = localStorage.getItem('token') || '';
        const response = await axios.get(`${BACKEND_URL}admin/pedidos/${email}`, {
            headers: {
                'x-auth-token': token
            }
        });
...
>('gerirUsers/submitUser', async (userData, { rejectWithValue }: any) => {
    try {
        const token = localStorage.getItem('token') || '';
        const response = await axios.put(`${BACKEND_URL}admin/user/${userData.idSelecionado}`, {
...
const GerirUserSlice = createSlice({
    ...
    extraReducers:{
        ...
        [fetchUserPedidos.fulfilled]:(state,action: PayloadAction<PedidoType[]>) => {
...
export const selectEmailSelecionado = (state: RootState) => state.gerirUsers.emailSelecionado;
export const selectIdSelecionado = (state: RootState) => state.gerirUsers.idSelecionado;
export const selectNomeSelecionado = (state: RootState) => state.gerirUsers.nomeSelecionado;
export const selectAlterarSenha = (state: RootState) => state.gerirUsers.alterarSenha;
